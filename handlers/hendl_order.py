...
async def buy_step1(callback_query: types.CallbackQuery):
    ...
    for i in all_product:
        ...
        if not prod or not int(prod[7]) or not int(prod[-1]):
            ...
            print('В корзине прописываем товыр в наличии 1')
            continue
        elif int(i[2]) > int(prod[7]) and int(prod[-1]):
            db.bd_checking_availability(int(prod[7]), 1, callback_query.from_user.id, i[1])
            print('изменяем количество товара в гавфе count_stock  и прописываем 1 в наличии')
...
async def buy_step2(user_id):
    ...
    if not user_prod:
        ...
        print('Заказываем все товары формируем заказ удаляем заказ из корзины, убираем количество из наличия'
              ' отправляем заказ на сборку')

        await order_formation(user_id)
    elif len(all_user_prod) > len(all_count):
        print('Заказ изменён подтвердите временно удаляем из наличия и отправляем пользователю на подтверждение ')
